...
print(Data)
print ("Sistema de Organização de bases -- VERIFICADOR")
print("v 1.02")
...
msg = easygui.msgbox("""ESSA APLICAÇÃO SOMENTE VERIFICA OS ARQUIVOS!
                     \nOs arquivos já estão na pasta?\nCertifique-se que:\n
...
for i in Files:
    ...
    if ext_check in ext_verific:
        ...
        #Renomeando arquivo
        nn = (os.path.join(Path_name,((tipo+" - "+transportadora+"_").upper())+(mes)+ext))
...
#print (print([i for i,x in enumerate (testador) if testador.count(x) >1]))
print ("FIM")
os.system("pause")
#FUNÇAO DE EMAIL <<<<<<<<<<<<<<<<<<<<<<<<<<<<<<>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
